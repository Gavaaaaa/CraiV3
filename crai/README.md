...
| **N-12** | `agent/workflow.py:201`, `churn_voluntary/voluntary_agent.py:118`, `dunning/dunning_engine.py:109`, `dunning/pix_automatico_retry.py:334`, `ml/anomaly_detector.py:145` | `UnicodeEncodeError` no console padrão do Windows (cp1252). **7 ocorrências restantes**, todas em strings que chegam ao stdout — `→` (U+2192) e os emoji `✅`/`❌` (U+2705/U+274C). Medido com `tokenize`, contando só literais fora de docstring. | Estas 7 **são** pré-existentes: estão idênticas em `baseline-pre-sprint`. Outras **5**, que este diff introduziu (`ml/failure_classifier.py:250` por `4109d84`, `integrations/payment_gateway.py:562` e as 3 de `scripts/preparar_amostra_real.py`), foram corrigidas neste sprint — `→` virou `->`. ⚠️ **`anomaly_detector.py:145` derruba `python -m crai.scripts.train_all`, que é o gate do Sprint 4**: ou o Sprint 4 fixa a saída em UTF-8 antes de treinar, ou o gate roda com `PYTHONIOENCODING=utf-8` declarado. A solução sistêmica (`sys.stdout.reconfigure`) é do `demo_runner.py` no Sprint 5, de que o gate GA2 depende. |
